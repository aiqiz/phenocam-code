...
def readin_time(fname, UTC=False):
    dt = False
...
def GCC_calc_global(image, data, width, height):
    ...
    # === Step 1: Create union mask for all ROIs ===
    ...
    # === Step 2: Extract all GCC pixels inside the union ===
...
def soft_interior_weight(d, e_mean, e_std, min_weight=0.05):
    w = np.ones_like(d)
...
def generate_soft_interior_mask(union_mask, e_mean, e_std, show_plot=False):
    """
...
def compute_weighted_gcc(image, weight_mask, show_heatmap=False):
    ...
    if len(gcc_values) == 0:
        print("⚠️ No valid pixels in ROI — possibly fully on black boundary.")
...
def gcc(folder_dir, image_folder_name, ref_image_name, transformation_csv, roi_json,
...
if __name__ == "__main__":
    folder_dir = "/Volumes/Aiqi_02/phenocams/tame_phenocams/phenocam01/"
    image_folder_name = 'data/'
    ref_image_name = 'reference_image_02.jpg'
    transformation_csv = 'transformation_reference_projective_02.csv'
    roi_json = 'reference_roi_02.json'
    ...
    df = gcc(folder_dir, image_folder_name, ref_image_name, transformation_csv, roi_json,
        time_threshold, sza_threshold, edge_density_threshold, 
        local_lattitude, local_longitude, inlier_ratio_threshold)
    
    df.to_csv(folder_dir + "gcc_global_02.csv", index=False)
